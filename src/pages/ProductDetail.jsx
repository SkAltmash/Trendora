    ...
    <div className="max-w-5xl mx-auto px-4 py-10 mt-20">
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-600 mb-4">Category: <span className="capitalize">{product.category}</span></p>
          <p className="text-gray-600 mb-4">Gender: {product.gender}</p>
          <p className="text-gray-600 mb-4">Available Quantity: {product.quantity ?? 'N/A'}</p>

          {product.quantity === 0 ? (
            <p className="text-red-600 font-semibold mb-2">Out of Stock</p>
          ) : product.quantity < 5 ? (
            <p className="text-yellow-600 font-medium mb-2">Hurry! Only {product.quantity} left in stock.</p>
          ) : null}
              ...
              <Link to={`/product/${item.id}`} key={item.id}>
                <div className="w-40 md:w-55 rounded overflow-hidden shadow-lg bg-white hover:shadow-xl transition duration-300 cursor-pointer">
                  <img
                    ...
                    src={item.id >= 31 ? item.mainImage?.trim() : `/${item.mainImage?.trim()}`}
                    alt={item.title}
                  />
